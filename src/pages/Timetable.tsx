...
const TimetableTable = ({ caption, rows }: { caption: string; rows: string[][] }) => (
    ...
    <table className="w-full border-collapse rounded-2xl overflow-hidden" style={{ boxShadow: "0 4px 16px hsla(284,33%,30%,0.06), 0 12px 32px hsla(284,33%,30%,0.04)" }}>
      ...
      <thead>
        <tr className="bg-primary text-primary-foreground">
          {columns.map((col) => (
            <th key={col} className="px-2 py-2.5 text-[10px] lg:text-xs font-semibold whitespace-nowrap text-center">
              {col}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={i} className={`transition-colors duration-200 ${i % 2 === 0 ? "bg-card" : "bg-secondary"} hover:bg-accent/50`}>
            {row.map((cell, j) => (
              <td key={j} className="px-2 py-1.5 text-[10px] lg:text-xs text-center whitespace-nowrap text-foreground border-b border-border">
                {cell}
              </td>
            ))}
          </tr>
